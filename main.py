...
class MedicalSRGApp(ctk.CTk):
    def __init__(self):
        super().__init__()
        self.title("Aplikasi Segmentasi Citra Medis: Seed Region Growing")
    ...
    def setup_gui(self):
        ...
        ctk.CTkLabel(self.sidebar, text="PANEL KONTROL", font=("Arial", 16, "bold")).pack(pady=10)

        self.btn_img = ctk.CTkButton(self.sidebar, text="Unggah Gambar", command=self.upload_gambar)
        self.btn_img.pack(pady=10, padx=20)

        self.btn_txt = ctk.CTkButton(self.sidebar, text="Unggah Anotasi", command=self.upload_anotasi)
        self.btn_txt.pack(pady=10, padx=20)

        ctk.CTkLabel(self.sidebar, text="Threshold:").pack(pady=(20,0))
        ...
        self.btn_run = ctk.CTkButton(self.sidebar, text="Jalankan Segmentasi", fg_color="green",
        ...
        self.canvas_label = ctk.CTkLabel(self.display_frame, text="Silakan unggah gambar medis")
    ...
    def upload_gambar(self):
        ...
        if path:
            ...
            self.lbl_seed.configure(text="Klik bagian pada gambar untuk pilih seed")
    ...
    def upload_anotasi(self):
        ...
        if path and self.img_asli is not None:
            ...
            ctk.CTkLabel(self.sidebar, text="Anotasi Ground Truth berhasil dimuat", text_color="cyan").pack()
    ...
    def process_segmentation(self):
        ...
        self.lbl_iou.configure(text=f"Hasil Evaluasi IoU:\nBaseline: {iou_b:.4f}\nAdaptif: {iou_a:.4f}")
    ...
    def calc_iou(self, pred):
        ...
        outer = np.logical_or(pred, self.ground_truth).sum()
        return inter / outer if outer != 0 else 0.0
    ...
    def show_final_window(self, m_base, m_adapt, iou_b, iou_a):
        ...
        axes[3].imshow(a_ov)
        axes[3].set_title(f"SRG Adaptive\nIoU: {iou_a:.4f}")
